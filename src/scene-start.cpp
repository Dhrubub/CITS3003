...
void loadTextureIfNotAlreadyLoaded(int i) {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, textures[i]->width,
			textures[i]->height, 0, GL_RGB, GL_UNSIGNED_BYTE,
			textures[i]->rgbData);
	CheckError();
...
void loadMeshIfNotAlreadyLoaded(int meshNumber) {
	...
	glBufferData(GL_ARRAY_BUFFER,
			sizeof(float) * (3 + 3 + 3) * mesh->mNumVertices,
			NULL, GL_STATIC_DRAW);
...
void init(void) {
	...
	sceneObjs[1].brightness = 15.0; //0.2; // The light's brightness is 5 times this (below).
...
void display(void) {
	...
	// move camera a distance r away from the center (DONE ABOVE)
	// glTranslatef(0, 0, -r);
	...
	for (int i = 0; i < nObjects; i++) {
		...
		vec3 rgb = so.rgb * lightObj1.rgb * so.brightness * lightObj1.brightness
				* 2.0;
...
static void lightMenu(int id) {
	...
	if (id == 70) {
		toolObj = 1;
		setToolCallbacks(adjustLocXZ, camRotZ(), adjustBrightnessY,
...
static int createArrayMenu(int size, const char menuEntries[][128],
		void (*menuFn)(int)) {
...
static void materialMenu(int id) {
	...
	if (id == 10) {
		toolObj = currObject;
		setToolCallbacks(adjustRedGreen, mat2(1, 0, 0, 1), adjustBlueBrightness,
				mat2(1, 0, 0, 1));
	}
...
void timer(int unused) {
	char title[256];
	sprintf(title, "%s %s: %d Frames Per Second @ %d x %d", lab, programName,
			numDisplayCalls, windowWidth, windowHeight);
...
void fileErr(char *fileName) {
	...
	printf(
			"When not in the CSSE labs, you will need to include the directory containing\n");
	printf(
			"the models on the command line, or put it in the res folder next to the executable.");
	exit(1);
}
